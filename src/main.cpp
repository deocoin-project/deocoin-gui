...
int main(int argc, char* argv[]) {
  ...
  QSettings deocoinKey("HKEY_CLASSES_ROOT\\deocoin", QSettings::NativeFormat);
  deocoinKey.setValue(".", "Deocoin Wallet");
  deocoinKey.setValue("URL Protocol", "");
  QSettings deocoinOpenKey("HKEY_CLASSES_ROOT\\deocoin\\shell\\open\\command", QSettings::NativeFormat);
  deocoinOpenKey.setValue(".", "\"" + QCoreApplication::applicationFilePath().replace("/", "\\") + "\" \"%1\"");
#endif
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Deocoin URL Handler\\nGenericName = Deocoin\\nComment = Handle URL Sheme deocoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/deocoin;\\nIcon = Deocoin-Wallet' | tee /usr/share/applications/deocoin-handler.desktop";
