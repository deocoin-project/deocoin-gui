...
void SendFrame::parsePaymentRequest(QString _request) {
    ...
    if(_request.startsWith("deocoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "deocoin:");
    }
    if(!_request.startsWith("deocoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with deocoin:"), QtCriticalMsg));
      return;
    }
...
void SendFrame::sendClicked() {
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "DEC1RALGaP5U8BLJskYR2YVSjr3DQEEuS5xghbtX2mm134YVXgS4RJHZGkeBvXf4BRFLWkv4zHGJ267S9pjwvVt63xwkdYPCwF";
